...
class plgVmPaymentPagsegurotransparente extends vmPSPlugin {
	...
	function __construct(& $subject, $config) {
		//if (self::$_this)
		//   return self::$_this;
		JLog::addLogger( array('text_file' => 'plg-pagseguro-transparente-j4-vm4.log.php'), JLog::DEBUG, array("errorRequest") );
	...
	function plgVmConfirmedOrder($cart, $order) {
		...
		//Sem isso não funciona com o OPC Rupustel e alguns temas.
		echo $html;
	}
	...
	function plgVmgetPaymentCurrency($virtuemart_paymentmethod_id, &$paymentCurrencyId) {
		...
		function getCosts(VirtueMartCart $cart, $method, $cart_prices) {
				if (preg_match('/%$/', $method->cost_percent_total)) {
						$cost_percent_total = (float)substr($method->cost_percent_total, 0, -1);
				} else {
						$cost_percent_total = (float)$method->cost_percent_total;
				}
				//return ($cart_prices['salesPrice'] * $cost_percent_total * 0.01);
				return ((float)$method->cost_per_transaction + ($cart_prices['salesPrice'] * $cost_percent_total * 0.01));
		}
		/**
		...
		public function plgVmOnShowOrderFEPayment($virtuemart_order_id, $virtuemart_paymentmethod_id, &$payment_name) {
			...
			if ($view == 'orders' and $orderDetails['details']['BT']->virtuemart_paymentmethod_id == $virtuemart_paymentmethod_id) {
				...
				if ($method->transacao_em_andamento == $orderDetails['details']['BT']->order_status) {
					...
					$idModal = $this->createModalJ4("Recibo", $url_recibo);
					$html .= '<h6>Prazos para aprovação do Pagamento via pagseguro: Cartão e Boleto 24h úteis Débito Online 2h.</h6>
								<button type="button" class="btn btn-link" data-bs-toggle="modal" data-bs-target="#modal-' . $idModal . '">
									Clique aqui para visualizar o status detalhado da transação no Pagseguro
								</button>';

					JFactory::getApplication()->enqueueMessage(
						$html, "Pagseguro Transparente Alert"
					);

					//Alterações copiadas do plugin rodando no site dev.organite-brasil.com
				} else if (
						($method->transacao_cancelada == $orderDetails['details']['BT']->order_status) 
						or 
						($method->transacao_em_andamento == $orderDetails['details']['BT']->order_status 
							and isset($dados_pagseguro[0]->transactionCode) 
							and $dados_pagseguro[0]->transactionCode == "")
					) {
	...
	function plgVmOnPaymentNotification() {
		...
		if ($task2 == 'redirecionarPagseguroAPI') {
			...
			$forma_pagamento  = vRequest::getVar('forma_pagamento');
			$tipo_pagamento   = vRequest::getVar('tipo_pagamento');
			// json de retorno js
			$json_retorno     = array();
			...
			JLog::add( "[plgVmOnPaymentNotification()] >> ".json_encode($pagseguro_data), JLog::DEBUG, 'errorRequest');
			/*
...
//              $meio_pagamento     	= $transacao_dados->transacao->meio_pagamento;
	...
	function plgVmOnPaymentResponseReceived(&$html='') {
		...
		if (!class_exists('VirtueMartModelOrders'))
			require( JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'orders.php' );
		$pagsegurotransparente_data = vRequest::get('post');
	...
	public function getCreditCard() {
		$arr_cartao         = array();
		$holder           = vRequest::getVar('c_holder');
		$number           = vRequest::getVar('c_number');
		$securityCode         = vRequest::getVar('c_securityCode');
		$expiry_date        = $this->getExpiryDate(vRequest::getVar('c_expiry_date'));
	...
	public function getParcelas() {
		$parcelas = vRequest::getVar('parcela_selecionada',1);
		return $parcelas;
	}
	...
	public function createTransaction() {
		...
		$token_pagseguro= $this->getToken($method);   
		$token_compra   = vRequest::getVar('token_compra');
		$forma_pagamento= vRequest::getVar('forma_pagamento');
		$tipo_pagamento = vRequest::getVar('tipo_pagamento');
		$senderHash   = vRequest::getVar('senderHash');
		...
		// if ($numero_metodo_pagamento) {
			...
			$url_notificacao              	= JROUTE::_(JURI::root() . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginnotification&tmpl=component&pm='.$order['details']['BT']->virtuemart_paymentmethod_id);
			...
			if (empty($cpfComprador)) {
				$cpfComprador               = $this->formataCPF(vRequest::getVar('cpf'));
			}
			if ($method->modo_debug) {
			...
			$json_pedido['shippingAddressPostalCode'] = str_replace('-','',$st_comprador->zip);
			...
			$valor_parcela                = vRequest::getVar('valor_parcela','');
			...
			$json_pedido['billingAddressPostalCode']  = str_replace('-','',$bt_comprador->zip);
			...
			//$json_pedido['notificationURL']       = 'https://www.homologacao.orbitalcommerce.com.br/retorno.php';
			...
			if ($forma_pagamento == 'CartaodeCredito') {
				...
				$cpf_form = vRequest::getVar('c_cpf');
				$json_pedido['senderCPF']         = $this->formataCPF($cpf_form);
				$c_holder                 = vRequest::getVar('c_holder');
				$c_cpf                    = $this->formataCPF($cpf_form);
				$c_phone                  = vRequest::getVar('c_phone');
				$c_birthdate              = vRequest::getVar('c_birthdate');
				// $birthDate             = $this->formataData(((isset($bt_comprador->$campo_data_nascimento) and !empty($bt_comprador->$campo_data_nascimento))?$bt_comprador->$campo_data_nascimento:''));                
			...
			JLog::add( "[createTransaction()] >> ".json_encode($json_pedido), JLog::DEBUG, 'errorRequest');
	...
	*/
	...
	public function trataRetornoSucessoPagseguro($retorno, $method) {
		...
		$tipo_pagamento     = vRequest::getVar('tipo_pagamento');
		$forma_pagamento    = vRequest::getVar('forma_pagamento');
				$parcela_selecionada  = vRequest::getVar('parcela_selecionada');
	...
	function createModalJ4($title = "Modal",$url = ""){
		$randomId = random_int(1, 1000000);
		$modalParams['url']    = $url;
		...
		$modalParams['footer'] = '<button type="button" class="btn" data-bs-dismiss="modal" data-bs-target="#closeBtn" aria-hidden="true">Fechar</button>';

		echo HTMLHelper::_('bootstrap.renderModal','modal-' . $randomId, $modalParams);
